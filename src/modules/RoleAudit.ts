...
export default class RoleAudit extends Module {
    ...
    async onEnable() {
        ...
        setInterval(() => void this.auditRoles(), timestring(config.role_audit_interval, "ms"));
        ...
        this.client?.on("roleDelete", async role => {
           ...
           await (this.client?.channels.cache.get(config.logs_channel) as TextChannel)!.send("@everyone a protected role was deleted.");
    ...
    async auditRoles(): Promise<void> {
        ...
        const roles: Role[] = Array.from(rolesCache.keys()).map(k => rolesCache.get(k)!);
        for (const role of roles) {
            ...
            if (modified)await role.setPermissions(permissions, "Role Audit").catch(() => undefined);
        }
