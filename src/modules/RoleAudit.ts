...
export default class RoleAudit extends Module {
  ...
  onEnable(): void {
    ...
    this.client?.on("roleDelete", async (role) => {
      ...
      if (
        !log?.executor ||
        !config.restricted_permissions_allowed_roles.includes(role.id) ||
  ...
  async auditRoles(): Promise<void> {
    return new Promise<void>(async (res) => {
      ...
      const roles: Array<Role> = Array.from(rolesCache.keys()).map((k) =>
        rolesCache.get(k as string),
      ) as Array<Role>;
      for (const role of roles) {
        ...
        if (modified)
          role.setPermissions(permissions, "Role Audit").catch((err) => {});
      }
      ...
      guild!.members.cache.forEach(async (member) => {
        ...
        if (!account) {
          if (hasVerifiedRole)
            await member.roles.remove(config.verified_role, "Role Audit");
          if (hasCitizenRole)
            await member.roles.remove(config.citizen_role, "Role Audit");
          return;
        }
