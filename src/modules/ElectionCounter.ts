...
export default class ElectionCounter extends Module {
  ...
  async commenceCount() {
    this.countNumber = 0;
    this.votes = {};
    this.candidates = (await ElectionCandidate.find()).map((c) => c.discordId!);
    ...
    if (winner) {
      ...
      for (const [i, candidates] of candidatesSortedByNumberOfVotes.entries()) {
        ...
        if (
          ...
          totalEliminatedVotes + candidates.length * i < nextNumberOfVotes
        ) {
          ...
          const toEliminate =
            candidatesSortedByNumberOfVotes[i][
              Math.floor(Math.random() * candidates.length - 0.001)
            ];
      ...
      outputMessageBuilder.push(
        `**${formatArrayValuesAsHumanReadableString(
          eliminating.map((c) => "<@" + c + ">"),
        )} ${eliminating.length > 1 ? "have" : "has"} been eliminated.**`,
      );
