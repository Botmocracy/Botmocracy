...
export default class Module {
    ...
    async handleCommand(command: Omit<CommandOptions, "cmdBuilder">, i: ChatInputCommandInteraction) {
        ...
        await i.reply({content: "You do not have permission to execute this command.", ephemeral: true});
    }
    ...
    initialise(client: Client<true>): void {
        ...
        this.client.on("interactionCreate", async i => {
            ...
            if (!Object.keys(this.slashCommands).includes(i.commandName)) return;
            const command: CommandOptions | SubcommandOptions = this.slashCommands[i.commandName]
            ...
            try {
                ...
                i.reply({ content: errorMessage, ephemeral: true }).catch(() => i.editReply({ content: errorMessage }));
            }
        })
        this.onEnable();
    }
