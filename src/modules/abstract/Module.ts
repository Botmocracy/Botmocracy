...
export default class Module {
  ...
  initialise(client: Client<true>): void {
    ...
    this.client.on("interactionCreate", async (i) => {
      ...
      this.logger.info(
        `Processing command ${i.commandName}${i.options.getSubcommand(false) ? " " + i.options.getSubcommand(false) : ""} with arguments ${data.options?.map((o) => `${o.name}:${o.value}`).join(" ")} from ${i.user.tag}`,
      );
