...
import { ChatInputCommandInteraction, GuildMember, RoleResolvable, SlashCommandBuilder, TextChannel } from "discord.js";
import { config } from "..";
            ...
            executor: async (i: ChatInputCommandInteraction) => {
               ...
               await i.deferReply({ ephemeral: true });
                ...
                try {
                   ...
                   await i.editReply({ content: "You need to verify with https://minecraftauth.me first." });
                    return;
                }
                ...
                if (name == null) {
                   await i.editReply({ content: "I couldn't get your username for some reason. Try again." });
                    return;
                }
                ...
                if (members.includes(name)) {
                   ...
                   await (this.client?.channels.cache.get(config.welcome_channel) as TextChannel).send(`${i.user} welcome! Check out <#995567687080091769> for information on joining.`);
                } else await i.editReply({ content: "You do not seem to be an MRT member." });
            }
        }
    }
}