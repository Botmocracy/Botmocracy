...
export default class Auth extends Module {
  ...
  async getMinecraftOrDiscordName(
    ...
    const minecraftUsername =
      await this.getMinecraftNameFromDiscordId(discordUserId);
  ...
  async getMembers() {
    ...
    data.forEach((player: any) => {
      ...
      res.push(
        ...player["Former Usernames"].split(",").map((n: string) => n.trim()),
      );
    });
  ...
  slashCommands = {
    verify: {
      ...
      executor: async (i: ChatInputCommandInteraction) => {
        ...
        if (name == null) {
          await i.editReply({
            content: "I couldn't get your username for some reason. Try again.",
          });
          return;
        }
        ...
        if (members.includes(name)) {
          ...
          void i.editReply({ content: "You do not seem to be an MRT member." });
      },
    },
  };
}
