        ...
        verify: {
            ...
            executor: async (i: ChatInputCommandInteraction) => {
                ...
                if (members.includes(name)) {
                    ...
                    void i.editReply({ content: "Verified!" });
                    void (this.client?.channels.cache.get(config.welcome_channel)! as TextChannel).send(`${i.user} welcome! Check out <#995567687080091769> for information on becoming a citizen.`);
                } else void i.editReply({ content: "You do not seem to be an MRT member." });
            }
        }
    }
}
