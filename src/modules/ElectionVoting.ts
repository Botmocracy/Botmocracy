...
export default class ElectionVoting extends Module {
  ...
  async getVotingPage(page: number, user: User): Promise<any> {
    return new Promise(async (res) => {
      ...
      if (page == 0 && !this.draftBallots.get(user.id)) {
        let previousVote = await ElectionVote.findOne({
          discordId: user.id,
        }).exec();
        if (previousVote) {
          this.draftBallots.set(
            user.id,
            previousVote.preferences! as unknown as Array<string>,
          );
        }
      }
      ...
      const electionPhase = await ElectionInfo.findOne().exec();
      if (electionPhase?.currentPhase != 2)
        res({ content: "Voting is not currently open.", components: [] });
  ...
  async recordVotePreference(i: SelectMenuInteraction) {
    ...
    const ballot = this.draftBallots.get(i.user.id)
      ...
      : Array(numberOfCandidates).fill(null);
  ...
  async confirmSubmitVote(i: ButtonInteraction) {
    ...
    if (!preferences)
      return i.update({
        content:
          "Your draft preferences seem to have not been saved. You will need to re-enter them by clicking on the `Vote` button again. Contact Dinty if this issue persists.",
      });
    ...
    this.logger.info(
      `Saved vote from ${i.user.tag} with ${preferences.length} preferences`,
    );
  }
}
